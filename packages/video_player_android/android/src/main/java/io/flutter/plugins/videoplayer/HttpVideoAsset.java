...
final class HttpVideoAsset extends VideoAsset {
  ...
  MediaSource.Factory getMediaSourceFactory(
    ...
    Map<String, String> headers = new java.util.HashMap<>(httpHeaders);
    boolean useReelCache = "1".equals(headers.remove(ReelCache.HEADER));
    unstableUpdateDataSourceFactory(initialFactory, headers, userAgent);
    DataSource.Factory dataSourceFactory = new DefaultDataSource.Factory(context, initialFactory);
    if (useReelCache) dataSourceFactory = ReelCache.factory(context, dataSourceFactory);
